...
export default function TrendingPage() {
    ...
    useGSAP(() => {
        ...
        tl.from(".trending-hero", {
            x: -50,
            opacity: 0,
            duration: 1.2,
            ease: "power3.out"
        })
            .from(".trending-header", {
                y: 20,
                opacity: 0,
                duration: 0.8,
                stagger: 0.1,
    ...
    return (
        <section ref={container} className="w-full flex flex-col lg:flex-row bg-white border border-gray-200 shadow-lg shadow-blue-100 overflow-hidden">

            {/* HERO IMAGE — shows first on mobile */}
            <div className="trending-hero order-1 lg:order-1 lg:w-1/2 w-full h-[50vh] sm:h-[55vh] md:h-[60vh] lg:h-screen relative">
                <Image
            ...
            <div className="order-2 lg:w-1/2 w-full flex flex-col p-5 sm:p-6 md:p-8 lg:p-12 bg-linear-to-b from-gray-50 to-white">
                ...
                {/* HEADER */}
                <div className="trending-header mb-6 sm:mb-8">
                ...
                <div className="space-y-3 sm:space-y-4 flex-1 overflow-y-auto pr-1 sm:pr-2 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                    ...
                    {ordered.slice(0, 3).map((blog, i) => (
                        <Link
                            key={blog.id}
                            href={`/blog/${blog.id}`}
                            className={`trending-card group block rounded-xl sm:rounded-2xl border p-3 sm:p-4 transition-all duration-300
              ${i === 0
                ...
                <Link
                    ...
                    className="trending-btn mt-6 sm:mt-8 relative inline-flex items-center justify-center gap-2 px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base font-semibold text-white overflow-hidden group rounded-xl sm:rounded-2xl"
                >
